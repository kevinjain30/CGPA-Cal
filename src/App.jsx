...
const ReportCard = React.forwardRef(({ semesters, cgpa, totalCredits }, ref) => (
    <div ref={ref} className="p-8 bg-white text-gray-800" style={{width: '600px', minHeight: '400px'}}>
        ...
        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-xl shadow-lg mb-6 flex justify-around items-center">
            <div className="text-center">
                <p className="text-sm uppercase font-semibold opacity-90">Overall CGPA</p>
                <p className="text-5xl font-bold">{cgpa}</p>
            </div>
            <div className="border-l-2 border-blue-300 h-16"></div>
            <div className="text-center">
                <p className="text-sm uppercase font-semibold opacity-90">Total Credits</p>
            ...
            {semesters.map((semester, index) => (
                <div key={index} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="font-bold text-lg text-gray-900">{semester.name}</h3>
                            <p className="text-sm text-gray-600">
                                {semester.subjects.reduce((acc, s) => acc + (parseFloat(s.credits) || 0), 0)} Credits
                            </p>
                        </div>
                        <div className="text-right">
                           <span className="font-bold text-2xl text-indigo-600">{semester.sgpa}</span>
        ...
        <div className="text-center mt-8 pt-4 border-t border-gray-200">
            <p className="text-xs text-gray-500">Report generated on {new Date().toLocaleDateString()}</p>
        </div>
    </div>
));

// --- Canvas-based image generation function ---
const generateImageFromElement = async (element) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Get computed styles and render text
            const renderElement = (el, offsetX = 0, offsetY = 0) => {
                const style = window.getComputedStyle(el);
                const rect = el.getBoundingClientRect();
                const elementRect = element.getBoundingClientRect();
                
                const x = rect.left - elementRect.left + offsetX;
                const y = rect.top - elementRect.top + offsetY;
                ...
                if (el.textContent && el.children.length === 0) {
                    ctx.fillStyle = style.color || '#000000';
                    ctx.fillText(el.textContent.trim(), x + 5, y + 5);
                }
...
export default function App() {
  ...
  const handleExport = async () => {
    ...
    try {
      // Show loading state
      setAlertMessage({ title: "Generating...", body: "Please wait while we generate your report image." });
      
      // Wait a bit for the loading message to show
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // Generate image using canvas
      const dataUrl = await generateImageFromElement(reportCardRef.current);
      ...
      if (navigator.share && navigator.canShare) {
        try {
          ...
          const file = new File([blob], 'cgpa-report.png', { type: 'image/png' });
      ...
      link.download = `cgpa-report-${new Date().toISOString().split('T')[0]}.png`;
      link.href = dataUrl;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
      setAlertMessage({ title: "Success", body: "Your CGPA report has been downloaded successfully!" });
      
    } catch (error) {
      console.error('Export error:', error);
      setAlertMessage({ title: "Export Error", body: "Could not generate the image. Please try again." });
    }
  };
          ...
          {semesters.length > 0 ? (
            <div className="space-y-4">
                {semesters.map((semester, index) => (
                  <div
                    key={index}
                    className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-5 flex items-center gap-4 transform transition-all duration-300 hover:shadow-lg"
                  >
